...
display_step = 10 #Cada 10 iteraciones se mostrara en pantalla info
...
# Metodo para crear la capa de convolucion
def conv2d(x, W, b, strides=1):
	x = tf.nn.conv2d(x, W, strides = [1, strides,strides, 1], padding = 'SAME') #Filtros de la convolucion, usamos ZeroPadding para que la salida sea del mismo tamano
...
def conv_net(x, weights, biases, dropout):
	...
	# Creamos primera capa convolucional
...
# Evaluamos modelo
correct_pred = tf.equal(tf.argmax(pred, 1), tf.argmax(y, 1)) #Comparamos los resultados entre la prediccion de la red y los  verdaderos outputs de y
